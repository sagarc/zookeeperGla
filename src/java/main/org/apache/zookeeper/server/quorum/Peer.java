//Code from Leader.java and updated by Sagar Chordia
...
//Use proposal class from Leader class instead of creating one new class
import org.apache.zookeeper.server.quorum.Leader.Proposal;
import org.apache.zookeeper.server.quorum.Learner.PacketInFlight;
...
public class Peer {
    ...
    //Use propose Class from Leader instead of using it from Peer
   ...
   /* static public class Proposal {
    ...
    // list of all the peer machines (GLA proposers/Learners and Acceptors)
    public final HashSet<PeerLearnerHandler> learners =
        new HashSet<PeerLearnerHandler>();

    // list of followers that are ready to follow (i.e synced with the leader)
    //They form list of peers (proposers, acceptors. learners)
    public final HashSet<PeerLearnerHandler> forwardingFollowers =
        new HashSet<PeerLearnerHandler>();
    
    //no use for our case
    protected final HashSet<PeerLearnerHandler> observingLearners =
        new HashSet<PeerLearnerHandler>();
    ...
    //Follower counter
    final static AtomicLong followerCounter = new AtomicLong(-1);
    /**
     ...
     */
    void addLearnerHandler(PeerLearnerHandler learner) {
    ...
    /**
     * Remove the peer from the learner list and acceptor list =?
     * 
     * @param peer
     */
    void removeLearnerHandler(PeerLearnerHandler peer) {
    ...
    boolean isLearnerSynced(PeerLearnerHandler peer){
        synchronized (forwardingFollowers) {
    ...
    Peer(QuorumPeer self,PeerZooKeeperServer zk) throws IOException {
    	System.out.println("130:Peer");
    	this.self = self;
        try {
            ss = new ServerSocket();
    ...
    /**
     * This message type is sent to a peer to request and mutation operation.
    ...
    /**
     * This message type is sent by a peer to all acceptors to propose a mutation.
     */
    public final static int PROPOSAL = 2;

    /**
     * This message type is sent by a follower(GLA:acceptor) after it has synced a proposal.
     */
    final static int ACK = 3;

    /**
     * This message type is sent by a peer to commit a proposal and cause
     * other peers to start serving the corresponding data.
     */
    final static int COMMIT = 4;

    /**
     * This message type is enchanged between follower(accpetor) and leader(peer) (initiated by
     ...
     * between the leader(peer) and the follower(accpetor).
     */
            ...
            try {
                while (!stop) {
                    try{
                        ...
                        PeerLearnerHandler fh = new PeerLearnerHandler(s, Peer.this);
                        fh.start();
    ...
    void lead() throws IOException, InterruptedException {
       // zk.registerJMX(new LeaderBean(this, zk), self.jmxLocalPeerBean);
        ...
        try {
            ...
            // new followers.
            //here in learnerCnxnAcceptor packets from other learners are fetched in leader server
            // but some modification is requried
            LOG.info("Starting to lead from peer");            
            ...
            //while (newLeaderProposal.ackCount <= self.quorumPeers.size() / 2) {
                if (self.tick > self.initLimit) {
                    ...
                    for(PeerLearnerHandler f : learners)
    ...
    void shutdown(String reason) {
        ...
        synchronized (learners) {
            for (Iterator<PeerLearnerHandler> it = learners.iterator(); it
                    .hasNext();) {
                PeerLearnerHandler f = it.next();
                it.remove();
                f.shutdown();
            }
        }
    ...
    /**
     * Keep a count of acks that are received by the peer for a particular
     * proposal
     * 
     * @param zxid
    ...
    synchronized public void processAck(long sid, long zxid, SocketAddress followerAddr) {
        ...
        if (self.getQuorumVerifier().containsQuorum(p.ackSet)){             
            ...
            if ((zxid & 0xffffffffL) != 0) {
                ...
                // Dont need to inform as observes is empty
                //inform(p);
    ...
    void sendPacket(QuorumPacket qp) {
    	LOG.info("sending packet to " + forwardingFollowers.size()+" followers");
        synchronized (forwardingFollowers) {
            for (PeerLearnerHandler f : forwardingFollowers) {                
                f.queuePacket(qp);
            }
        }
    }
    ...
    void sendObserverPacket(QuorumPacket qp) {        
        synchronized(observingLearners) {
            for (PeerLearnerHandler f : observingLearners) {                
                f.queuePacket(qp);
            }
        }
    }
    ...
    public void commit(long zxid) {
        ...
        QuorumPacket qp = new QuorumPacket(Peer.COMMIT, zxid, null, null);
        sendPacket(qp);
    }
    ...
    public Proposal propose(Request request) throws XidRolloverException {
        ...
        QuorumPacket pp = new QuorumPacket(Peer.PROPOSAL, request.zxid, 
    ...
    /**
     * lets the peer know that a follower(another follower) is capable of following and is done
     * syncing
     * 
     * @param handler handler of the follower
     * @return last proposed zxid
     */
    synchronized public long startForwarding(PeerLearnerHandler handler,
            long lastSeenZxid) {
        ...
        if (lastProposed > lastSeenZxid) {
            for (Proposal p : toBeApplied) {
                ...
                // commit message also
                QuorumPacket qp = new QuorumPacket(Peer.COMMIT, p.packet
                        .getZxid(), null, null);
